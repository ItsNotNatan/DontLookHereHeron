...
export default function CardEditavel({ atm, onCancelar, onSalvar }) {
  ...
  // ESTADOS CONTROLADOS PARA AUTOFILL DE ENDEREÇO
  ...
  // ESTADOS DA LISTA DE CARGAS (JSON)
  ...
  const dataEntregaFormatada = atm.data_entrega ? atm.data_entrega.split('T')[0] : '';
  const dataColetaFormatada = atm.data_coleta ? atm.data_coleta.split('T')[0] : '';
  const dataSolFormatada = atm.data_solicitacao ? atm.data_solicitacao.split('T')[0] : '';

  // TRUQUE DAS MEDIDAS TOTAIS
  ...
  // BUSCA OS DADOS DE APOIO (TRANSPORTADORAS, PROJETOS, LOCAIS E MOTIVOS)
  useEffect(() => {
    const carregarDadosApoio = async () => {
      try {
        const [resTransp, resProj, resLocais, resMotivos] = await Promise.all([
          ...
          api.get('/admin/motivos') // 🌟 Busca os motivos
        ]);
        ...
        // 🌟 Formata os motivos com cor e descrição para o Select
  ...
  // LÓGICA DE AUTO-FILL DA ENTREGA
  ...
  // FUNÇÕES PARA GERENCIAR ITENS DA CARGA
  ...
  const handleEditCarga = (id, campo, valor) => {
    setCargas(cargas.map(carga => 
      carga.id === id ? { ...carga, [campo]: valor } : carga
    ));
  };

  // 🌟 NOVO: Função para renderizar o interior do dropdown de motivos com cor
  const formatarMotivoOption = ({ label, color, description }, { context }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '4px 0' }}>
        ...
        {description && (
          <span style={{ fontSize: '0.8rem', color: '#64748b', marginLeft: '20px', lineHeight: 1.3 }}>
            {description}
          </span>
        )}
      </div>
  ...
  const handleSubmit = async (e) => {
    ...
    const dadosParaEnviar = {
      ...
      motivo: d.motivo, // 🌟 Apanha automaticamente do input hidden
  ...
  return (
    <form className="card-editavel" onSubmit={handleSubmit}>
      <div className="card-editavel__content">
        ...
        {/* SEÇÃO 1: INFORMAÇÕES GERAIS E STATUS */}
        <h4 className="card-editavel__section-title">Informações Gerais</h4>
        <div className="card-editavel__grid card-editavel__grid--cols-4">
          ...
          <div className="card-editavel__grid-item--span-4" style={{ position: 'relative' }}>
        ...
        {/* SEÇÃO 2: FINANCEIRO E NF */}
        <h4 className="card-editavel__section-title">Financeiro & NF</h4>
        <div className="card-editavel__grid card-editavel__grid--cols-4">
          <div>
          ...
          <div>
            ...
            <input type="number" step="0.01" name="valor_nf" defaultValue={atm.valor_nf} className="card-editavel__input" placeholder="0.00" />
          </div>
          <div>
            <label className="card-editavel__label">Valor Frete (R$)</label>
            <input type="number" step="0.01" name="valor_frete" defaultValue={atm.valor || atm.valor_frete} className="card-editavel__input" placeholder="0.00" />
          </div>
          <div>
          ...
          <div className="card-editavel__address-box">
            ...
            <div style={{ marginBottom: '1rem' }}>
              <Select
                ...
                placeholder="Selecione um local cadastrado..."
                isSearchable
                isClearable
                styles={reactSelectStyles}
              />
        ...
        {/* SEÇÃO 4.1: ITENS INDIVIDUAIS DA CARGA */}
        ...
        <div style={{ backgroundColor: '#f8fafc', padding: '1.25rem', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
          ...
          {/* LISTAGEM EDITÁVEL DOS ITENS JÁ EXISTENTES */}
          {cargas.length > 0 && (
            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <hr style={{ border: 'none', borderTop: '1px dashed #cbd5e1', marginBottom: '10px' }} />
              <label className="card-editavel__label">Itens Adicionados (Edite diretamente abaixo)</label>
                  ...
                  <div style={{ flex: 1, paddingRight: '15px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                      <div style={{ flex: 2 }}>
                        ...
                        <input type="text" value={carga.nome} onChange={e => handleEditCarga(carga.id, 'nome', e.target.value)} className="card-editavel__input" placeholder="Nome do Item" style={{ padding: '4px 8px' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label className="card-editavel__label" style={{ fontSize: '0.65rem', marginBottom: '2px', color: '#64748b' }}>Qtd</label>
                        <input type="number" min="1" value={carga.quantidade} onChange={e => handleEditCarga(carga.id, 'quantidade', e.target.value)} className="card-editavel__input" placeholder="Qtd" style={{ padding: '4px 8px' }} />
                      </div>
                      <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <input type="number" step="0.01" min="0" value={carga.peso} onChange={e => handleEditCarga(carga.id, 'peso', e.target.value)} className="card-editavel__input" placeholder="Peso(kg)" style={{ padding: '4px 8px' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label className="card-editavel__label" style={{ fontSize: '0.65rem', marginBottom: '2px', color: '#64748b' }}>Comp. (m)</label>
                        <input type="number" step="0.01" min="0" value={carga.comprimento} onChange={e => handleEditCarga(carga.id, 'comprimento', e.target.value)} className="card-editavel__input" placeholder="C(m)" style={{ padding: '4px 8px' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label className="card-editavel__label" style={{ fontSize: '0.65rem', marginBottom: '2px', color: '#64748b' }}>Larg. (m)</label>
                        <input type="number" step="0.01" min="0" value={carga.largura} onChange={e => handleEditCarga(carga.id, 'largura', e.target.value)} className="card-editavel__input" placeholder="L(m)" style={{ padding: '4px 8px' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label className="card-editavel__label" style={{ fontSize: '0.65rem', marginBottom: '2px', color: '#64748b' }}>Alt. (m)</label>
                        <input type="number" step="0.01" min="0" value={carga.altura} onChange={e => handleEditCarga(carga.id, 'altura', e.target.value)} className="card-editavel__input" placeholder="A(m)" style={{ padding: '4px 8px' }} />
                      </div>
                    </div>
        ...
        <div className="card-editavel__grid card-editavel__grid--cols-4">
          <div className="card-editavel__grid-item--span-2" style={{ position: 'relative' }}>
            ...
            <Select
              ...
              value={opcoesTransportadoras.find(opt => opt.value === transportadoraSelecionada) || (transportadoraSelecionada ? { value: transportadoraSelecionada, label: transportadoraSelecionada } : null)}
              onChange={(selecionado) => setTransportadoraSelecionada(selecionado ? selecionado.value : '')}
              placeholder={opcoesTransportadoras.length === 0 ? "Carregando..." : "Buscar transportadora..."}
              isSearchable
              isClearable
              styles={reactSelectStyles}
            />
          ...
          <div className="card-editavel__grid-item--span-2">
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="number" step="0.01" min="0" name="medida_c" defaultValue={compAtual} className="card-editavel__input" placeholder="Comp." title="Comprimento" style={{ textAlign: 'center' }} />
              <input type="number" step="0.01" min="0" name="medida_l" defaultValue={largAtual} className="card-editavel__input" placeholder="Larg." title="Largura" style={{ textAlign: 'center' }} />
              <input type="number" step="0.01" min="0" name="medida_a" defaultValue={altAtual} className="card-editavel__input" placeholder="Alt." title="Altura" style={{ textAlign: 'center' }} />
            </div>
          </div>
        </div>

        {/* SEÇÃO 5: RASTREIO E OBSERVAÇÕES */}
        ...
        <div className="card-editavel__grid card-editavel__grid--cols-2">
          ...
          <div style={{ position: 'relative' }}>
            ...
            {/* 🌟 NOVO: Select dos Motivos com campo hidden */}
            ...
            <Select
              ...
              isClearable
              styles={reactSelectStyles}
              formatOptionLabel={formatarMotivoOption} // 🌟 A magia do design entra aqui
            />
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.5rem', marginTop: '10px' }}>
          ...
          <textarea 
            name="observacoes" 
            defaultValue={atm.observacoes} 
            className="card-editavel__input" 
            rows="3" 
            placeholder="Anotações sobre a carga, alertas, etc..."
