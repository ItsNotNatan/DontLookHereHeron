...
export default function CardExpandido({ atm, onClose, onAtmUpdated }) {
  ...
  const gerarPayloadSeguro = (novosDados) => {
    ...
    return {
      ...
      valor: atm.valor || atm.faturamento?.valor,
      ...
      // Mantém as listas atuais se não forem atualizadas
      ...
      // Sobrescreve com as atualizações que passarmos
      ...dadosPreparados
    };
  };
  ...
  // --- LÓGICA MESTRA: UPLOAD, DRIVE E SUPABASE AUTOMÁTICO ---
  ...
  const processarArquivos = async (arquivos) => {
    ...
    if (envioComSucesso) {
      try {
        // Salva os links no banco e força o status "Entregue"
              ...
              <div className="card-expandido__grid">
                ...
                <div>
                  ...
                  <ul className="card-expandido__list">
                    ...
                    <li className="card-expandido__list-item"><span className="card-expandido__label">Medidas (CxLxA):</span> <strong className="card-expandido__value">{atm.medidas || 'Não informado'}</strong></li>
                    ...
                    <li className="card-expandido__list-item"><span className="card-expandido__label">Tipo de Frete:</span> <strong className="card-expandido__value">{atm.tipo_frete || 'Não informado'}</strong></li>
                  ...
                  <div className="card-expandido__tracking-box" style={{ display: 'flex', gap: '12px', alignItems: 'stretch', marginTop: '1rem', backgroundColor: '#fff5f5', borderColor: '#fecaca' }}>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      <Select
                        ...
                        placeholder={opcoesMotivos.length === 0 ? "Carregando motivos..." : "Selecione uma divergência (opcional)..."}
                        isClearable
                        styles={{
                          ...reactSelectStyles,
                          control: (base, state) => ({
